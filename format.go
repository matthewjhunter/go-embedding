...
//
// The prefix is followed by a newline so that structured field labels
// (FormatRecord output) appear at column 0 on the line below the
// prefix, rather than running on after it. This keeps recurring
// "key:" positional patterns stable across the corpus, which is the
// learnable signal structured input is meant to provide.
...
// pipe-separated header of the form "task: <description> | query:" for
// query-style tasks, and "title: <title> | text:" for document
// indexing. Source: EmbeddingGemma model card.
//
...
//
// Each prompt is followed by a newline so structured field labels start
// at column 0 below the prefix rather than continuing on the same line.
...
// document prompt: "title: <title> | text:\n<text>". Empty title falls
