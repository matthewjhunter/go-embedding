...
func TestLookupTaskPrompter_TagSuffixFallsBackToBase(t *testing.T) {
	...
	for _, model := range cases {
		t.Run(model, func(t *testing.T) {
			...
			if got := p(TaskClustering, "x"); !strings.Contains(got, "clustering") {
...
func TestNomicTaskPrompter(t *testing.T) {
	cases := []struct {
		...
		{TaskRetrievalDocument, "search_document: hello"},
		{TaskRetrievalQuery, "search_query: hello"},
		{TaskClassification, "classification: hello"},
		{TaskClustering, "clustering: hello"},
...
func TestGemmaTaskPrompter(t *testing.T) {
	cases := []struct {
		...
		{TaskRetrievalDocument, "title: none | text: hello"},
		{TaskRetrievalQuery, "task: search result | query: hello"},
		{TaskClustering, "task: clustering | query: hello"},
		{TaskClassification, "task: classification | query: hello"},
		{TaskSimilarity, "task: sentence similarity | query: hello"},
		{TaskQuestionAnswering, "task: question answering | query: hello"},
		{TaskFactChecking, "task: fact checking | query: hello"},
		{TaskCodeRetrieval, "task: code retrieval | query: hello"},
	}
	for _, tc := range cases {
		t.Run(string(tc.task), func(t *testing.T) {
...
func TestFormatForTask(t *testing.T) {
	...
	if got != "clustering: x" {
		t.Errorf("got %q, want %q", got, "clustering: x")
	}
	got = FormatForTask("embeddinggemma", TaskClustering, "x")
	if got != "task: clustering | query: x" {
		t.Errorf("got %q, want %q", got, "task: clustering | query: x")
	}
}
...
func TestFormatRecordForTask(t *testing.T) {
	...
	want := "clustering: feed: Schneier on Security\nauthor: Bruce Schneier\n\nFull body."
	if got != want {
		t.Errorf("got %q\nwant %q", got, want)
	}
}
...
func TestRegisterTaskPrompter_ExactWinsOverTagFallback(t *testing.T) {
	...
	if got := FormatForTask("nomic-embed-text", TaskClustering, "x"); got != "clustering: x" {
...
func TestFormatRetrievalDocument_GemmaWithTitle(t *testing.T) {
	...
	want := "title: My Real Title | text: feed: X\nauthor: Y\n\nBody."
	if got != want {
		t.Errorf("got %q\nwant %q", got, want)
	}
}
...
func TestFormatRetrievalDocument_NomicNoSlot(t *testing.T) {
	...
	want := "search_document: title: My Title\nfeed: X\n\nBody."
	if got != want {
		t.Errorf("got %q\nwant %q", got, want)
	}
}
